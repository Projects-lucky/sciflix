...
import { PersonCard } from '@/components/person/person-card-context';
...
async function fetchPopularPeople(
  ...
  const response = await fetch(
    `/api/tmdb/person/popular?${query.toString()}`,
...
export function PersonClient() {
  return (
    <InfiniteScroll<TMDBPerson>
      ...
      renderItem={(person) => <PersonGridCard className="w-full h-84 md:w-58 md:h-99 border-none" key={person.id} person={person} />}
      emptyState={
        <SearchEmpty variant="no-results" query="popular people" />
      }
